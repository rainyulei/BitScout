...
fn bench(label: &str, pattern: &str, dir: &Path) {
    ...
    println!(
        ...
        rg_min.as_secs_f64() * 1000.0,
        rg_m
    );
    println!(
        "  {:>18} | {:>7.2}ms | {:>7.2}ms | {:>8}",
        "BitScout",
        bs_avg.as_secs_f64() * 1000.0,
        bs_min.as_secs_f64() * 1000.0,
        bs_m
    );
    ...
    if ratio > 1.0 {
        ...
        println!(
            "  => rg {:.1}x faster (process startup amortized)",
            1.0 / ratio
        );
    }
}
...
fn bench_binary_only(label: &str, pattern: &str, dir: &Path) {
    ...
    println!(
        "  BitScout: avg {:.2}ms, min {:.2}ms => {} matches (text + gz + docx)",
        bs_avg.as_secs_f64() * 1000.0,
        bs_min.as_secs_f64() * 1000.0,
        bs_m
    );
    println!(
        "  ripgrep:  {} matches (text only, MISSES {} binary files)",
        rg_m,
        bs_m.saturating_sub(rg_m)
    );
}
...
fn main() {
    ...
    // Run benchmarks
    bench(
        "1. Common keyword — small corpus",
        "authenticate",
        small.path(),
    );
    bench(
        "2. Rare keyword — medium corpus",
        "UNIQUE_SEARCH_TARGET_NEEDLE",
        medium.path(),
    );
    bench(
        "3. Common keyword — medium corpus",
        "authenticate",
        medium.path(),
    );
    bench_binary_only(
        "4. Binary file search — BitScout exclusive",
        "UNIQUE_SEARCH_TARGET_NEEDLE",
        binary.path(),
    );
    ...
    bench(
        "5. Real project — BitScout source code",
        "extract_text",
        Path::new("/Users/rainlei/holiday/BitScout"),
    );
