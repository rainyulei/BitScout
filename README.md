...
| 搜索 `.pdf` | ✗ | ✓ 自动提取文本 |
| BM25 评分排序 | ✗ | ✓ (`--bm25`) |
| 语义搜索 | ✗ | ✓ (`--semantic`, Random Projection) |
...
─────────────────────────── ─────── ──────
bitscout-core unit           110    ✓ all pass
...
semantic accuracy (e2e)        9    ✓ all pass
benchmark modes (e2e)          4    ✓ all pass
─────────────────────────── ─────── ──────
Total                        225+   ✓ all pass
```
...
基于 Random Projection 的语义搜索，无需 Embedding 模型：

- **Johnson-Lindenstrauss 引理**：随机投影保持向量距离
- **零依赖**：内置 PRNG (xorshift128+) + Box-Muller 变换，不依赖 `rand`/`ndarray`
- **SIMD 加速**：投影和 cosine similarity 均用 SIMD 内核
- **冷启动友好**：每个文件独立投影，无需全局索引
...
crates/
  bitscout-core/       # 核心引擎
    src/
      search/          # SearchEngine, Matcher, BM25, RP, SIMD
...
- **Scoring**: BM25 (TF/IDF) + Random Projection (cosine similarity)
